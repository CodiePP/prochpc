...
defaultfp = "./dist-newstyle/build/x86_64-osx/ghc-8.6.4/my-proj-0.1.0.0/hpc/vanilla/html/my-proj-0.1.0.0/hpc_index_fun.html"

findRows :: Cursor -> [Cursor]
findRows = element "body" >=> child >=> element "table" >=> child >=> element "tr" >=> child

extractModules :: Cursor -> [T.Text]
extractModules = element "td" >=> child >=> element "tt" >=> child >=> element "a" >=> child >=> content
extractScores :: Cursor -> [T.Text]
extractScores = element "td" >=> child >=> content
